...
function copyText(ele){new ClipboardJS(ele,{text:function(trigger){return ele.getAttribute("data-value")}}).on('success',function(e){alert("Successfully copied link");e.clearSelection()}).on('error',function(e){alert('Error!')})}
...
async function btcSign(){try{let accounts=await window.wizz.requestAccounts();console.log("account:",accounts[0])}catch(e){let res=await window.wizz.getAccounts();console.log("account:",res)}
let pubkey=await window.wizz.getPublicKey();console.log("pubkey:",pubkey)
try{let res=await window.wizz.signMessage("Sign in to this forum, your data is secure!");console.log(res)
$.post("./index.php?action=btcsign",{sign:res,pubkey:pubkey},function(result){console.log(result)
if(result.status!=0){window.location.reload()}else{alert(result.error)}})}catch(e){console.log(e)}}
...
if(typeof ethereum!="undefined"){ethereum.on('accountsChanged',function(accounts){if(smf_member_id!=0){window.location.href=smf_scripturl+'?action=logout;'+smf_session_var+'='+smf_session_id+';u='+smf_member_id}})};var smf_formSubmitted=!1;var lastKeepAliveCheck=new Date().getTime();var smf_editorArray=new Array();var ua=navigator.userAgent.toLowerCase();var is_opera=ua.indexOf('opera')!=-1;var is_ff=(ua.indexOf('firefox')!=-1||ua.indexOf('iceweasel')!=-1||ua.indexOf('icecat')!=-1||ua.indexOf('shiretoko')!=-1||ua.indexOf('minefield')!=-1)&&!is_opera;var is_gecko=ua.indexOf('gecko')!=-1&&!is_opera;var is_chrome=ua.indexOf('chrome')!=-1;var is_safari=ua.indexOf('applewebkit')!=-1&&!is_chrome;var is_webkit=ua.indexOf('applewebkit')!=-1;var is_ie=ua.indexOf('msie')!=-1&&!is_opera;var is_ie11=ua.indexOf('trident')!=-1&&ua.indexOf('gecko')!=-1;var is_iphone=ua.indexOf('iphone')!=-1||ua.indexOf('ipod')!=-1;var is_android=ua.indexOf('android')!=-1;var ajax_indicator_ele=null;if(!('forms' in document))
...
{$('#'+stripId).append('<a href="'+options.sUrl+'" class="button last" '+('sCustom' in options?options.sCustom:'')+' '+('sId' in options?' id="'+options.sId+'_text"':'')+'>'+options.sText+'</a>')};function smf_StatsCenter(oOptions)
{this.opt=oOptions;this.oTable=null;this.oYears={};this.bIsLoading=!1;this.init()}
smf_StatsCenter.prototype.init=function()
{this.oTable=document.getElementById(this.opt.sTableId);if(typeof(this.oTable)!='object')
return;var aRows=this.oTable.getElementsByTagName('tr');var aResults=[];var sYearId=null;var oCurYear=null;var sMonthId=null;var oCurMonth=null;for(var i=0,n=aRows.length;i<n;i++)
{if((aResults=this.opt.reYearPattern.exec(aRows[i].id))!=null)
{sYearId=aResults[1];this.oYears[sYearId]={oCollapseImage:document.getElementById(this.opt.sYearImageIdPrefix+sYearId),oMonths:{}};oCurYear=this.oYears[sYearId];oCurYear.bIsCollapsed=oCurYear.oCollapseImage.src.indexOf(this.opt.sYearImageCollapsed)>=0;oCurYear.oToggle=new smc_Toggle({bToggleEnabled:!0,bCurrentlyCollapsed:oCurYear.bIsCollapsed,instanceRef:this,sYearId:sYearId,funcOnBeforeCollapse:function(){this.opt.instanceRef.onBeforeCollapseYear(this)},aSwappableContainers:[],aSwapImages:[{sId:this.opt.sYearImageIdPrefix+sYearId,srcExpanded:smf_images_url+'/'+this.opt.sYearImageExpanded,altExpanded:'-',srcCollapsed:smf_images_url+'/'+this.opt.sYearImageCollapsed,altCollapsed:'+'}],aSwapLinks:[{sId:this.opt.sYearLinkIdPrefix+sYearId,msgExpanded:sYearId,msgCollapsed:sYearId}]})}
else if((aResults=this.opt.reMonthPattern.exec(aRows[i].id))!=null)
{sMonthId=aResults[1];oCurYear.oMonths[sMonthId]={oCollapseImage:document.getElementById(this.opt.sMonthImageIdPrefix+sMonthId)};oCurMonth=oCurYear.oMonths[sMonthId];oCurMonth.bIsCollapsed=oCurMonth.oCollapseImage.src.indexOf(this.opt.sMonthImageCollapsed)>=0;var sLinkText=getInnerHTML(document.getElementById(this.opt.sMonthLinkIdPrefix+sMonthId));oCurMonth.oToggle=new smc_Toggle({bToggleEnabled:!0,bCurrentlyCollapsed:oCurMonth.bIsCollapsed,instanceRef:this,sMonthId:sMonthId,funcOnBeforeCollapse:function(){this.opt.instanceRef.onBeforeCollapseMonth(this)},funcOnBeforeExpand:function(){this.opt.instanceRef.onBeforeExpandMonth(this)},aSwappableContainers:[],aSwapImages:[{sId:this.opt.sMonthImageIdPrefix+sMonthId,srcExpanded:smf_images_url+'/'+this.opt.sMonthImageExpanded,altExpanded:'-',srcCollapsed:smf_images_url+'/'+this.opt.sMonthImageCollapsed,altCollapsed:'+'}],aSwapLinks:[{sId:this.opt.sMonthLinkIdPrefix+sMonthId,msgExpanded:sLinkText,msgCollapsed:sLinkText}]});oCurYear.oToggle.opt.aSwappableContainers[oCurYear.oToggle.opt.aSwappableContainers.length]=aRows[i].id}
else if((aResults=this.opt.reDayPattern.exec(aRows[i].id))!=null)
{oCurMonth.oToggle.opt.aSwappableContainers[oCurMonth.oToggle.opt.aSwappableContainers.length]=aRows[i].id;oCurYear.oToggle.opt.aSwappableContainers[oCurYear.oToggle.opt.aSwappableContainers.length]=aRows[i].id}}
...
this.oYears[oToggle.opt.sYearId].oMonths[sMonth].oToggle.changeState(!0)}
smf_StatsCenter.prototype.onBeforeCollapseMonth=function(oToggle)
{if(!oToggle.bCollapsed)
{getXMLDocument(smf_prepareScriptUrl(smf_scripturl)+'action=stats;collapse='+oToggle.opt.sMonthId+';xml');var aNewContainers=[];var oYearToggle=this.oYears[oToggle.opt.sMonthId.substr(0,4)].oToggle;for(var i=0,n=oYearToggle.opt.aSwappableContainers.length;i<n;i++)
if(!in_array(oYearToggle.opt.aSwappableContainers[i],oToggle.opt.aSwappableContainers))
aNewContainers[aNewContainers.length]=oYearToggle.opt.aSwappableContainers[i];oYearToggle.opt.aSwappableContainers=aNewContainers}}
smf_StatsCenter.prototype.onBeforeExpandMonth=function(oToggle)
{if(this.bIsLoading)
return;if(oToggle.opt.aSwappableContainers.length==0)
{sendXMLDocument.call(this,smf_prepareScriptUrl(smf_scripturl)+'action=stats;expand='+oToggle.opt.sMonthId+';xml','',this.onDocReceived);if('ajax_indicator' in window)
ajax_indicator(!0);this.bIsLoading=!0}
else getXMLDocument(smf_prepareScriptUrl(smf_scripturl)+'action=stats;expand='+oToggle.opt.sMonthId+';xml')}
smf_StatsCenter.prototype.onDocReceived=function(oXMLDoc)
{var aMonthNodes=oXMLDoc.getElementsByTagName('month');for(var iMonthIndex=0,iNumMonths=aMonthNodes.length;iMonthIndex<iNumMonths;iMonthIndex++)
{var sMonthId=aMonthNodes[iMonthIndex].getAttribute('id');var iStart=document.getElementById('tr_month_'+sMonthId).rowIndex+1;var sYearId=sMonthId.substr(0,4);var aDayNodes=aMonthNodes[iMonthIndex].getElementsByTagName('day');for(var iDayIndex=0,iNumDays=aDayNodes.length;iDayIndex<iNumDays;iDayIndex++)
{var oCurRow=this.oTable.insertRow(iStart+iDayIndex);oCurRow.className=this.opt.sDayRowClassname;oCurRow.id=this.opt.sDayRowIdPrefix+aDayNodes[iDayIndex].getAttribute('date');for(var iCellIndex=0,iNumCells=this.opt.aDataCells.length;iCellIndex<iNumCells;iCellIndex++)
{var oCurCell=oCurRow.insertCell(-1);if(this.opt.aDataCells[iCellIndex]=='date')
oCurCell.className='stats_day';var sCurData=aDayNodes[iDayIndex].getAttribute(this.opt.aDataCells[iCellIndex]);oCurCell.appendChild(document.createTextNode(sCurData))}
this.oYears[sYearId].oMonths[sMonthId].oToggle.opt.aSwappableContainers[this.oYears[sYearId].oMonths[sMonthId].oToggle.opt.aSwappableContainers.length]=oCurRow.id;this.oYears[sYearId].oToggle.opt.aSwappableContainers[this.oYears[sYearId].oToggle.opt.aSwappableContainers.length]=oCurRow.id}}
this.bIsLoading=!1;if(typeof(window.ajax_indicator)=='function')
ajax_indicator(!1)}